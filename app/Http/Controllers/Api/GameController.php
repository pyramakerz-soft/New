   ...
   public function game(Request $request)
{
    ...
    $gamesQuery = Game::with([
            'gameImages', 'gameLetters', 'gameTypes',
            'lesson.unit.program.course', 'lesson.unit'
        ]);
   ...
   public function gamebyId(Request $request)
{
    ...
    if ($request->filled('game_id')) {
        ...
        if (!$game) return response()->json(['message' => 'No games found for the provided lesson'], 404);
    }
    ...
    $gamesQuery = Game::with([
            'gameImages','gameLetters','gameChoices',
            'gameTypes','lesson.unit.program.course','lesson.unit','studentDegrees'
        ])
        ...
        $games_id = Game::selectRaw('MIN(id) AS id')
            ->where(function ($q) use ($lessonId) {
                $q->where('lesson_id', $lessonId)
                  ->orWhere('adaptive_lesson_id', $lessonId)
                  ->orWhere('sec_adaptive_lesson_id', $lessonId);
            })
